...
const templates = [
  // Kontrak Bisnis
  {
    name: 'Perjanjian Sewa Menyewa Tempat',
  ...
  },
  {
    ...
    icon: 'sale',
    available: true,
    href: 'jual-beli-barang-bergerak/landing.html',
  },
...
function createTemplateCard(template) {
  ...
  const productUrl = isAvailable ? (template.href || `${slug}.html`) : '#';
...
function performSearch(query) {
  ...
  if (lowerQuery && visibleCount > 0) {
    const templatesSection = document.getElementById('katalog-dokumen');
    if (templatesSection) {
      setTimeout(() => {
...
// Deep link: index.html#Kontrak%20Bisnis → tab kategori yang cocok
(function syncHashToCategoryTab() {
  ...
  function apply() {
    var key = normHash(typeof location !== 'undefined' ? location.hash : '');
    if (!key) return;
    var tabs = document.querySelectorAll('.category-tab[data-category]');
    var i;
    for (i = 0; i < tabs.length; i++) {
      var cat = (tabs[i].getAttribute('data-category') || '').toLowerCase();
      if (cat === key) {
        tabs[i].click();
        return;
      }
    }
    if (key === 'perjanjian sewa') {
      for (i = 0; i < tabs.length; i++) {
        if ((tabs[i].getAttribute('data-category') || '') === 'Kontrak Bisnis') {
          tabs[i].click();
          return;
        }
      }
